...
def create_model(hps, vocab_size, num_classes):
    ...
    with tf.variable_scope('lstm_nn_posts', initializer=lstm_init):
        cells = []
        ...
        rnn_outputs, _ = tf.nn.dynamic_rnn(
            cell, embed_posts, initial_state=initial_state)

        last_posts = rnn_outputs[:, -1, :]

    with tf.variable_scope('lstm_nn_responses', initializer=lstm_init):
        cells = []
        ...
        # rnn_outputs: [batch_size, num_timesteps, lstm_outputs[-1]]
        rnn_outputs, _ = tf.nn.dynamic_rnn(
            cell, embed_responses, initial_state=initial_state)

        last_responses = rnn_outputs[:, -1, :]

    # global_maxpooling_posts， global_maxpooling_responses -> concat_maxpooling
    concat_last = tf.concat([last_posts, last_responses], axis=1)

    fc_init = tf.uniform_unit_scaling_initializer(factor=1.0)

    with tf.variable_scope('fc', initializer=fc_init):
        fc1 = tf.layers.dense(concat_last,
                              hps.num_fc_nodes,
